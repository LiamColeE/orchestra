...
async function main() {
  ...
  async function loop() {
    const harness = new MultiAgentHarness(config, config);

    harness.onEvent((event) => {
      const { role, type, content } = event;
      const prefix = role === 'orchestrator' ? `${CYAN}[${role}]${RESET}` :
        role === 'planner' ? `${YELLOW}[${role}]${RESET}` :
          role === 'builder' ? `${GREEN}[${role}]${RESET}` :
            role === 'reviewer' ? `${YELLOW}[${role}]${RESET}` :
              role === 'janitor' ? `${GRAY}[${role}]${RESET}` :
              `${DIM}[${role}]${RESET}`;
      if (type === 'text') {
        renderer.handle({ type: 'text', delta: content });
      } else if (type === 'tool_call') {
        renderer.handle({ type: 'tool_call', name: role, callId: '', args: { info: content } });
      } else {
    ...
    while (true) {
      ...
      try {
        const result = await harness.processUserPrompt(trimmed);
        loader.stop();
        renderer.endTurn();

        messages.push({ role: 'assistant', content: result });
        saveMessage(sessionPath, { role: 'assistant', content: result });

        console.log(`\n${GRAY}  Multi-agent harness complete${RESET}\n`);
      } catch (err: any) {
        loader.stop();
        renderer.endTurn();
